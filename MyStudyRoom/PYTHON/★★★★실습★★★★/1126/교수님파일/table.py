...
class Table:
    ...
    def __init__(self, window, colour="black", net_colour="green",
                 width=800, height=600, vertical_net=False, horizontal_net=False):
        self.width = width
        self.height = height
        self.colour = colour
        
        # tkinter 공장으로부터 캔버스 주문: 생성한 테이블에  캔버스 붙이기
        ...
        # 득점판 추가:
        font = ("monaco", 60) 
        self.scoreboard = self.canvas.create_text(self.width/2, 50, font=font, fill = "darkgreen")
    ...
    def draw_rectangle(self, rectangle):
        ...
        c = rectangle.colour
        return self.canvas.create_rectangle(x1, y1, x2, y2, fill=c) #테이블(자기자신) 캔버스에 사각형 만들기 
    ...
    def draw_oval(self, oval):
        ...
        c = oval.colour
        return self.canvas.create_oval(x1, y1, x2, y2, fill=c) #테이블(자기자신) 캔버스에 원 만들기 
    
    # canvas의 아이템을 조작할 수 있는 도구: move_item(item이름에 따라..)
    def move_item(self, item, x1, y1, x2, y2):
        self.canvas.coords(item, x1, y1, x2, y2)
    
    def remove_item(self, item): #캔버스에서 삭제할 아이템
