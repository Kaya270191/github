...
def game_flow():                              #공을 일정시간마다 움직임
    ...
    if(my_ball.x_posn <= 0 ):
        ...
        if(score_right >=3):            #score_right가 3점 이상이면  
            score_right = "W"           #score_right 를 W로 표시 
            score_left = "L"                #score_right 를 L로 표시
...
def restart_game(master):
    ...
    if(score_left =="W" or score_right =="R"):
        score_left =0
        score_right =0
    my_table.draw_score(score_left, score_right)
...
#bgSpace=PhotoImage(file="background_space.png")
#bgLabel=Label(window, image=bgSpace)
#bgLabel.place(x=0,y=0)
    
my_table = Table(window, 600, 400 ,"pink","black")
