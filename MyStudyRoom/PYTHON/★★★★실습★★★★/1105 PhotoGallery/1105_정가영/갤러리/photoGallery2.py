...
btnPrev = Button(window, image=btnPrev1, command = clickPrev) #버튼 위젯 생성 -step1
btnNext = Button(window, image=btnNext1, command = clickNext)


labelText = Label(window, text =" Jeju1.gif", font = ("명조체",15)) #라벨 텍스트 생성 

photo = PhotoImage(file= fnameList[0]) #이미지를 파일로 부터 준비 -step1
pLabel = Label(window, image = photo) #이미지 위젯 생성 -step2

buttonExit = PhotoImage(file="../gif/xicon.png")
button1=Button(window, image = buttonExit, command=quit)


#엑스 아이콘 위젯 생성 
#
#btnExit = Label (window, image=buttonExit)
...
#btnExit.place(x=80,y=40)# 종료 위젯 디스플레이
