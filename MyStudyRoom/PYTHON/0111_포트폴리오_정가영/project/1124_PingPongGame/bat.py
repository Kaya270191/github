...
class Bat:
    def __init__(self, table, width, height, x_posn, y_posn, color, x_speed = 23, y_speed  =23): #생성자 
        ...
        self.x_start = x_posn           #배트의 시작x좌표값
        self.y_start = y_posn           #배트의 시작y좌표값
        self.x_speed = x_speed      #배트의 x속도 좌표값
        self.y_speed = y_speed      #배트의  y속도 좌표값
    ...
    def move_up(self, master):
        self.y_posn = self.y_posn - self.y_speed  #y_speed의 값 만큼 y_posn 값을 빼야 배트가 위로 올라감

        if(self.y_posn <=0):                    #bat가 위 화면에 닿으면 더이상 올라가지 않게 하는 코드 
            self.y_posn =0                      #bat_y포지션값이 0
    ...
    def move_down(self, master): #  이벤트(마우스, 키보드)를 처리해야하는 함수는 파라미터에 반드시 master나 event를 넣어줘야함
        self.y_posn = self.y_posn + self.y_speed  #y_speed의 값 만큼 y_posn 값을 더함

        if(self.y_posn >=300): #bat가 아래 화면에 닿으면 더이상 내려가지 않게 하는 코드 
            self.y_posn =300
        ...
        self.table.move_item(self.rectangle, x1, y1, x2, y2)        #table 클래스의 move_item() 실행 


    #배트를  오른쪽으로 움직이는 함수
    def move_right(self, master):
        self.x_posn = self.x_posn + self.x_speed  #x의 포지션= x 포지션+x의 속도 

        if(self.x_posn >=585):              #오른쪽 벽을 넘지 않으려면  창의 가로길이 600-배트의 가로길이 15를 뺀 좌표
            self.x_posn = 585                   #585보다 크면 x포지션 값을 585로 만들어서 창을 벗어나지 않게 함 
        x1 = self.x_posn
        x2 = self.x_posn + self.width
        y1 = self.y_posn
        y2 = self.y_posn + self.height
    ...
    def move_left(self, master):
        ...
        if(self.x_posn <=0):         #배트의 x 포지션이 0보다 더 작으면 
            self.x_posn = 0          #x 포지션 값을 0으로 만들기 
        x1 = self.x_posn
        x2 = self.x_posn + self.width
        y1 = self.y_posn
        y2 = self.y_posn + self.height

        self.table.move_item(self.rectangle, x1, y1, x2, y2)        #table클래스의 move_item() 함수 실행 
    ...
    def detect_collision(self, ball):
        ...
        if((bottom_b > top) and (top_b < bottom) and (right_b > left) and (left_b < right)):    #이 상태가 collision 임 
            collision = True                    #collision의 값 변경
            self.height -= 10                   #충돌시 배트의 길이가 작아짐
            print("충돌")
        ...
        if(collision):                   #만약 collision 이 True 라면, 즉 충돌했다면
            ...
            if((right_b > right) and (left_b <= right) and (top_b > top-r) and (bottom_b < bottom +r)):
               ...
               ball.x_speed = (ball.x_speed)+2 #공의 속도를 +2 증가
            ...
            #공의 왼쪽 부분이 배트의 왼쪽 부분보다 작고, 공의 오른쪽 부분이 배트의 왼쪽보다 크다면 ->배트의 서쪽에서 공이 충돌 
               ...
               ball.x_speed = (ball.x_speed)-2          #충돌시 공의 속도를 -2 증가
        ...
        return (collision, collision_direction)          #충돌 시 (충돌, 충돌방향)을 리턴 
    ...
    def start_position(self):
        ...
        self.height = 100               #배트의 높이 초기값을 설정해줌
        x1 = self.x_posn
        y1 = self.y_posn
        x2 = self.x_posn + self.width
        y2 = self.y_posn + self.height
        self.table.move_item(self.rectangle, x1,y1,x2,y2)       #table의 move_item()호출 
