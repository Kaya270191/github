...
const multiplyBySix = [] //빈배열
for(let i=1; i<100; i++){ //6의 배수
...
function addMovieInfo(movie, index, movie){
...
const removedFruits=fruits.splice(1,0,'banana') //1번자리에 삭제안함 바나나추가
console.log(fruits)
console.log(removedFruits) //제거한 요소들이 담겨져 있는 배열을 리턴 

const removedFruits1 = fruits.splice(1,0,'coconut','strawberry')//1번자리에 삭제안함, 코코넛 스트로베리 추가
console.log(fruits)
console.log(removedFruits1) //제거한 요소가 담긴 배열을 리턴

const removedFruits2 = fruits.splice(2, 1) //2번자리에서 시작 1개 요소 제거 
console.log(fruits)
console.log(removedFruits2)

const removedFruits3 = fruits.splice(0, 3, 'pear', 'lemon')//0번째자리에서부터 3개 요소 제거하고 pear lemon추가
console.log(fruits)
console.log(removedFruits3)//제거된 요소 배열로 리턴


const removedFruits4 = fruits.splice(fruits.length - 4, 2)//배열의 길이에서 시작해서 4번째, 두 개 제거
console.log(fruits)
...
const removedFruits7 = fruits7.splice(3, 1, 'lemon')//3번 인덱스에서 1개의 요소를 제거,레몬추가
console.log(fruits7)
